...
- **Hybrid storage.** Reference data (muscles, modalities, equipment) is TypeScript in `src/lib/data/` with id unions — never move it to the DB. Mutable data (clients, exercises + their muscle scores/modality variants, routines, programs, program_days, assignments, sessions, set_logs, weigh_ins) is Supabase; schema lives in `supabase/schema.sql` (run by hand in the SQL editor — the publishable key can't DDL). Clients live in the DB (so people can be added/edited at `/users`): `ClientId` is an open string, the TS roster in `src/lib/data/clients.ts` is seed + read-only fallback (`GymData.clientsSource === "seed"` while the clients table doesn't exist), and client ids are validated at write time via `assertClientId` in `src/lib/actions/clients.ts`, not a compile-time union. Client card colors come from the `CLIENT_COLORS` presets only — never free hex. Exercises follow the same pattern (authored at `/exercises`): `ExerciseId` is an open string, `src/lib/data/exercises.ts` is seed + read-only fallback (`GymData.exercisesSource === "seed"`; the three catalog tables fall back as a unit), ids are validated at write time via `assertExerciseIds` in `src/lib/actions/exercises.ts`, and the DB is seeded via the Import button at `/exercises`, never SQL inserts. The catalog reaches client components only as the serializable `ExerciseCatalog` slice (plain arrays via `catalogSlice`; lookups rebuilt with the WeakMap-cached `exerciseLookup` in `src/lib/exercise-catalog.ts`) — never pass full `GymData` or Maps across the boundary. Muscle scores are authored as roles (`SCORE_ROLES`: primary 10 / secondary 7 / supporting 4 / stabilizer 2, ≤2 primaries, ≤9 muscles — enforced in `saveExercise`) so hand-added exercises can't inflate the heat map; seed rows keep hand-tuned values.
...
- **Writes = server actions** in `src/lib/actions/`. The browser never imports the Supabase client. Every action validates text FKs against the unions via `src/lib/validate.ts` (exercise ids against the live table via `assertExerciseIds`) and ends with `revalidatePath("/", "layout")`.
...
- **Status color is never the only encoding** — pair it with a text label and legend (`StatusLabel`/`MeterLegend` in `src/components/meter-rows.tsx`); red↔green alone is colorblind-hostile. Ordinal (hip-band) work renders as the body heat map's hatched indicator (`src/components/body-heatmap.tsx`), never as length in a lbs bar.

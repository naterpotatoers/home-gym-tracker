...
- Mobile-first: nav is a top bar on `md:+` and a fixed bottom tab bar below (`PageShell`'s `pb-24` clears it). Dark mode is media-query driven (`color-scheme: light dark` set on `:root`); the heat ramp (`--heat-0..12`, reversed for dark) is unchanged and stays out of `@theme`.
